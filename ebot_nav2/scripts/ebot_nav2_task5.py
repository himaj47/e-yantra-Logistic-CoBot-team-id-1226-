...
class NavigationDockingController(Node):
    ...
    def initial_pose_rec(self):
        ...
        initial.pose.position.x=3.55
        initial.pose.position.y=-2.70
    ...
    def initial_pose_conv_1(self):
        initial=PoseStamped()
        initial.header.frame_id='map'
        initial.header.stamp=self.navigator.get_clock().now().to_msg()
        initial.pose.position.x=3.40
        initial.pose.position.y=-1.22
        initial.pose.position.z=0.0
        q=self.yaw_to_quaternion(self.reset_deg)
        initial.pose.orientation.x=q[0]
        initial.pose.orientation.y=q[1]
        ...
        initial.pose.orientation.w=q[3]
        self.navigator.setInitialPose(initial)
        self.get_logger().info(f'Set Intial Pose at Conveyor 1 Pose')

    def initial_pose_conv_2(self):
        initial=PoseStamped()
        initial.header.frame_id='map'
        initial.header.stamp=self.navigator.get_clock().now().to_msg()
        initial.pose.position.x=3.40
        initial.pose.position.y=1.84
        initial.pose.position.z=0.0
        q=self.yaw_to_quaternion(self.reset_deg)
        initial.pose.orientation.x=q[0]
        initial.pose.orientation.y=q[1]
